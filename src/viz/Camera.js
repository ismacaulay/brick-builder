...
export class Camera {
  constructor(object, canvas) {
    ...
    this._initialPosition = this._object.position.clone();
    this._initialZoom = this._object.zoom;
  }

  reset() {
    this._target.copy(this._initialTarget);
    this._object.position.copy(this._initialPosition);
    this._object.zoom = this._initialZoom;
  ...
  pan(deltaX, deltaY) {
    ...
    this._xPanVector.setFromMatrixColumn(this._object.matrix, 0);
    ...
    this._yPanVector.setFromMatrixColumn(this._object.matrix, 1);
    this._yPanVector.multiplyScalar(panUpDisatance);
    this._updatePanOffset.add(this._yPanVector);
  }

  update() {
    this._updateOffset.copy(this._object.position).sub(this._target);
    ...
    this._object.position.copy(this._target).add(this._updateOffset);
    ...
    if (
      this._zoomChanged ||
      this._lastPosition.distanceToSquared(this._object.position) > 0.000001 ||
      8 * (1 - this._lastQuaternion.dot(this._object.quaternion)) > 0.000001
    ) {
      this._lastPosition.copy(this._object.position);
